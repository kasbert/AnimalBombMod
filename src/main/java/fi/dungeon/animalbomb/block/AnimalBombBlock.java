...
import fi.dungeon.animalbomb.init.ModConfig;
...
public abstract class AnimalBombBlock extends Block {
	public AnimalBombBlock() {
		super(Material.TNT);
	...
	// Create animal
	abstract protected Entity createAnimal(World worldIn, BlockPos pos);

	private void spawn(World worldIn, BlockPos pos) {
		double x, y, z;
		x = (double) ((float) pos.getX() + 0.5F);
		y = (double) pos.getY();
		z = (double) ((float) pos.getZ() + 0.5F);
		worldIn.createExplosion(null, x, y + (double) (1 / 16.0F), z, 0.1F, true);
		for (int i = 0; i < ModConfig.numberOfAnimals; i++) {
			Entity entity = createAnimal(worldIn, pos);
			entity.setPosition(x, y, z);
			entity.prevPosX = x;
			entity.prevPosY = y;
			entity.prevPosZ = z;
			float f = (float) (Math.random() * (Math.PI * 2D));
			entity.motionX = (double) (-((float) Math.sin((double) f)) * 0.2F);
			entity.motionY = 0.5F + Math.random() * 0.5;
			entity.motionZ = (double) (-((float) Math.cos((double) f)) * 0.2F);
			entity.setFire(ModConfig.burningDuration);
			worldIn.spawnEntity(entity);
		}
	}

	// Code copied from TNT
	/**
	...
	public void onBlockDestroyedByExplosion(World worldIn, BlockPos pos, Explosion explosionIn) {
		if (!worldIn.isRemote) {
			...
			// entitytntprimed.setFuse((short)(worldIn.rand.nextInt(entitytntprimed.getFuse()
			// / 4) + entitytntprimed.getFuse() / 8));
	...
	public void explode(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase igniter) {
		if (!worldIn.isRemote) {
			if (((Boolean) state.getValue(EXPLODE)).booleanValue()) {
				...
				// EntityTNTPrimed(worldIn, (double)((float)pos.getX() + 0.5F),
				// (double)pos.getY(), (double)((float)pos.getZ() + 0.5F),
				// igniter);
				// worldIn.spawnEntity(entitytntprimed);
				// worldIn.playSound((EntityPlayer)null, entitytntprimed.posX,
				// entitytntprimed.posY, entitytntprimed.posZ,
